...
router.post('/auctions/:auctionID/bid', async (req, res, next) => {
  var t = curTimeInSec();
  console.log(t,"POST request to /auctions/", req.params.auctionID, "/bid from", req.ip, "with body params:",req.body)
  try {
...
router.get('/auctions/:auctionID', limiter, async (req, res, next) => {
  var t = curTimeInSec();
  try {
    console.log(t,"GET request to /auctions/", req.params.auctionID, "from", req.ip)
    if (req.params.auctionID == null || (!/\S/.test(req.params.auctionID))) {
      return res.send({"status":"false","message":"Please provide an auction id"});
    }
...
router.get('/auctions/:auctionID/bids', limiter, async (req, res, next) => {
  var t = curTimeInSec();
  try {
    console.log(t,"GET request to /auctions/", req.params.auctionID,"/bids from", req.ip)
    if (req.params.auctionID == null || (!/\S/.test(req.params.auctionID))) {
      return res.send({"status":"false","message":"Please provide an auction id"});
    }
    // get file with key from fleek
